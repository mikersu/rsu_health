...
class about_model extends CI_Model 
{
    ...
    public function add( $info = '' )
    {
        ...
        $title = ( ! empty( $info['slug'] ) ) ? $info['slug'] : reset($info['submenu']) ;
        ...
        $this->db->insert( 'about_config' );

        $last_id = $this->db->insert_id();

        foreach ( $info['submenu'] as $key => $value ) {
            $this->db->set( 'submenu', $value );
            $this->db->set( 'title', $value2 );
            $this->db->set( 'detail', $info['detail'][$key] );
            $this->db->set( 'language_id', $key );
            $this->db->set( 'about_id', $last_id );
            $this->db->insert( 'about_detail' );
    ...
    public function edit( $id = '' , $info = '' )
    {
        ...
        foreach ( $info['submenu'] as $key => $value ) {
            ...
            $this->db->update( 'about_detail' );

        }


        $this->db->where( 'about_id', $id );
        $this->db->delete( 'about_gallery' );

        if ( ! empty( $info['name_image'] ) ) {


            foreach ( $info['name_image'] as $key => $value ) {
                $this->db->set( 'about_id', $id );
                $this->db->set( 'image', $value );
                $this->db->insert( 'about_gallery' );
            }
        }
    ...
    {
        $this->db->where( 'id', $id );
        $query = $this->db->get( 'about_config' );
        $data_config = $query->row();

        $this->db->where( 'about_id', $id );
        $query = $this->db->get( 'about_detail' );
        $data = $query->result();
        ...
        $info['about_date'] = date( 'd/m/Y' , $data_config->about_date );
