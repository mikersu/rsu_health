...
class account_permission extends admin_controller {
	...
	function index() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_permission_perm', 'account_permission_manage_perm' ) != true ) { echo '<script>alert( "Account cannot access !! " ); history.go(-1);</script>';die();}
		...
		$output['hover_sub_menu'] = 'Setting Account Permission';

		$output['this_title_page'] = '<i class="icon-cogs"></i> Setting Account Permission';
		$breadcrumb = array( 'Home' => site_url('site-admin') , 'Setting Account Permission' => site_url('site-admin/account-permission') );
