...
class about extends MY_Controller {
	...
	function __construct() 
	{
		...
		$this->load->model( 'about/fabout_model' );
	...
	function index( $string_slug = '' ) 
	{
		...
		$data_list = $this->fabout_model->get_list( $this_lang , 'front' );
		...
		$output['gallery_list'] = ( ! empty( $show_data ) ) ? $this->fabout_model->get_gallery( $show_data->id ) : array() ;

		// --------------
		$data_tag = $this->seotag_model->this_tag( 'page_about_us' , 'home_page_about_us' , $this_lang );
		...
		foreach ( $data_tag as $key => $value ) 
		{
			if ( $value->tag_name == 'title' ) 
			{
				...
				$output['page_title'] = 'About us &rsaquo; '.$title;
			}
			else
			{
	...
	public function detail( $id = '' )
	{
		...
		//$output['calendar_list'] = $this->fabout_model->calendar_list( $start , $end );
		

		$output['show_data'] = $this->fabout_model->get_data( $id,'array');
		$output['data_list'] = $this->fabout_model->get_list( $this->session->userdata( 'lang' ) , 'front' );
		...
		$this_lang = $this->session->userdata( 'lang' );
		$this_lang = ( ! empty( $this_lang ) ) ? $this_lang : $this->lang_model->get_lang_default() ;
		$data_tag = $this->seotag_model->this_tag( 'about' , 'about_'.$id , $this_lang );
		foreach ( $data_tag as $key => $value ) 
		{
			if ( $value->tag_name == 'title' ) 
			{
		...
		$output['gallery_list'] = $this->fabout_model->get_gallery( $id,'array');
		$output['page_name'] = 'about';
