...
class account_level extends admin_controller {
	...
	function add() {
		...
		$output['hover_menu'] = 'Setting';
		$output['hover_sub_menu'] = 'Setting Position Account';
		$output['this_title_page'] = "Setting Position";

		$breadcrumb = array( 'Home' => site_url('site-admin') , 'Setting Position' => site_url('site-admin/account-level')  );
	...
	function index() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_lv_perm', 'account_lv_manage_perm' ) != true ) { echo '<script>alert( "Account cannot access !! " ); history.go(-1);</script>';die();}	
