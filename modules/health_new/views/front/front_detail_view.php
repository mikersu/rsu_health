            ...
            <article>
            	<div class="container header_page">
                    ...
                    <div class="breadcrumb">
						...
						<a href="<?php echo site_url('health_new');?>"><?php echo lang_get('Health_new'); ?></a>
                        <a href="<?php echo site_url('health_new/detail/'.$show_data['id']);?>" class="active"><?php echo $show_data['title'];?></a>
                    </div>
                </div>
                <div class="main_content default detail" id="health_new">
                    <div class="container">
						<div class="head">
                            <h1>
                                <span class="Helve_BdC page_name"><?php echo $show_data['title'];?></span>
                                <span class="date"><?php echo date('d/m/Y',$show_data['health_new_date']);?></span>
                            </h1>
                            <?php
							// $next='';
							// $previous='';
							...
							// 			$next='<a href="'.site_url('health_new/detail/'.$value->id).'" class="Helve_M">'.lang_get('Health_new Previous').'</a>';
							...
							// 		if($previous==''){
							// 			if($value->order_sort < $show_data['order_sort']){
							// 			$previous='<a href="'.site_url('health_new/detail/'.$value->id).'" class="Helve_M">'.lang_get('Health_new Next').'</a>';
							// 			}
							// 		}
							...
							// 	if($previous=='')
							// 	$previous='<a href="javascript:void(0);" class="Helve_M disable">'.lang_get('Health_new Previous').'</a>';
							// 	echo $previous;
							// 	if($next=='')
							// 	$next='<a href="javascript:void(0);" class="Helve_M disable">'.lang_get('Health_new Next').'</a>';
							// 	echo $next;
							// 	echo '</div>';
							// }
                            ...
                            foreach ( $data_list as $key => $value ) {
                                ...
                                if ( $value->health_new_id == $show_data['health_new_id'] ) {
                                    
                                    if ( ! empty( $data_list[$key-1]->health_new_id ) ) {
                                        echo $Previous='<a href="'.site_url('health_new/detail/'.$data_list[$key-1]->health_new_id).'" class="Helve_M">'.lang_get('Health_new Previous').'</a>';
                                    }
                                    if ( ! empty( $data_list[$key+1]->health_new_id ) ) {
                                        echo $next = '<a href="'.site_url('health_new/detail/'.$data_list[$key+1]->health_new_id).'" class="Helve_M">'.lang_get('Health_new Next').'</a>';
                                    }
