...
class health_new_model extends CI_Model 
{
    ...
    public function get_gallery( $id = '' , $type = 'object' )
    {
        ...
        $query = $this->db->get( 'health_new_gallery' );
    ...
    public function add( $info = '' )
    {
        ...
        $this->db->insert( 'health_new_config' );
        ...
        foreach ( $info['title'] as $key => $value ) {
            ...
            $this->db->set( 'health_new_id', $last_id );
            $this->db->insert( 'health_new_detail' );
        ...
        if ( ! empty( $info['name_image'] ) ) {
            foreach ( $info['name_image'] as $key => $value ) {
                ...
                $this->db->insert( 'health_new_gallery' );
            }
        }
    ...
    public function edit( $id = '' , $info = '' )
    {
        ...
        foreach ( $info['title'] as $key => $value ) {
            ...
            $this->db->update( 'health_new_detail' );

        }


        $this->db->where( 'health_new_id', $id );
        $this->db->delete( 'health_new_gallery' );

        if ( ! empty( $info['name_image'] ) ) {


            foreach ( $info['name_image'] as $key => $value ) {
                $this->db->set( 'health_new_id', $id );
                $this->db->set( 'image', $value );
                $this->db->insert( 'health_new_gallery' );
            }
        }
    ...
    public function get_list()
    {
        ...
        $this->db->from( 'health_new_config AS nc' );
        $this->db->join( 'health_new_detail AS nd', 'nc.id = nd.health_new_id', 'left' );
    ...
    {
        $this->db->where( 'id', $id );
        $query = $this->db->get( 'health_new_config' );
        $data_config = $query->row();

        $this->db->where( 'health_new_id', $id );
        $query = $this->db->get( 'health_new_detail' );
        $data = $query->result();
        ...
        $info['health_new_date'] = date( 'd/m/Y' , $data_config->health_new_date );
